...
(function ($) {
	...
	$.jstree.plugin("grid", {
		__init : function () { 
			...
			this.idTreeView = $(this.get_container()).attr("id");
		...
		_fn : { 
			_prepare_headers : function() {
				...
				cHeight, hHeight, container = this.get_container(), parent = container.parent(), hasHeaders = 0, conf = this.data.grid.defaultConf;
				widthGrid = $("#"+ this.idTreeView).width(), positionGrid = $("#"+ this.idTreeView).position();
				heightGrid = $("#"+ this.idTreeView).height() - 20;
				$("#"+ this.idTreeView).css({top : 20, left : 0, height : heightGrid});
				$("#"+ this.idTreeView).height($("#"+ this.idTreeView).height() + 20)
				// save the original parent so we can reparent on destroy
				this.data.grid.parent = parent;
				// set up the wrapper, if not already done
				header = this.data.grid.header || $("<div id='jstree-grid-header-"+ this.idTreeView +"'></div>").addClass("jstree-grid-header");
				// create the headers
				...
				if (hasHeaders) {
					try {
						$("<div style='width : " + widthGrid + "px; position: absolute; left: " + positionGrid.left + "px; top : " + positionGrid.top + "px;' id='jstree-grid-wrapper-"+ this.idTreeView +"'></div>").addClass("jstree-grid-wrapper").appendTo(parent).append(header).append(container);
